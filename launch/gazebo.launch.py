...
def generate_launch_description():
    ...
    spawn_carga_bot_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(join(carga_bot_path, "launch", "carga_bot_gazebo_spawn.launch.py")),
    )
    ...
    return LaunchDescription([
        ...
        value="/usr/share/gazebo-11:" + join(carga_bot_path, "worlds")),
        DeclareLaunchArgument('world', default_value = world_file),
        DeclareLaunchArgument('gui', default_value='true'),
        DeclareLaunchArgument('verbose', default_value='false'),
        DeclareLaunchArgument('use_sim_time', default_value = use_sim_time),
        gazebo,spawn_carga_bot_node
    ])
